...
export default function VerifyEmailContent() {
  ...
  useEffect(() => { if (!userId) router.push('/register') }, [userId, router])
  useEffect(() => {
  ...
  const handleDigitChange = (index: number, value: string) => {
    ...
    if (digit && index === 5) { const full = next.join(''); if (full.length === 6) submitOTP(full) }
  }
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (!pasted) return
    const next = [...digits]; pasted.split('').forEach((ch, i) => { if (i < 6) next[i] = ch })
    setDigits(next)
  ...
  const submitOTP = async (otp: string) => {
    ...
    try {
      ...
      setSuccess(true); setAuth(data.token, data.user)
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setResendMsg('New code sent! Check your inbox.'); setCountdown(60)
        ...
        <div className="bg-card/60 backdrop-blur-xl border border-border/60 rounded-2xl p-8 shadow-2xl shadow-black/30">
          {success ? (
            <div className="text-center py-4">
              ...
              <p className="text-muted-foreground text-sm">Your account is confirmed. Redirecting…</p>
            ...
            <>
              <div className="text-center mb-6">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed">
                  We sent a 6-digit code to {emailParam ? <strong className="text-foreground">{emailParam}</strong> : 'your email'}.
                </p>
              </div>
              ...
              {resendMsg && <div className="mb-4 p-3 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 text-sm text-center">{resendMsg}</div>}
              <form onSubmit={e => { e.preventDefault(); const otp = digits.join(''); if (otp.length < 6) { setError('Enter all 6 digits'); return } submitOTP(otp) }} className="space-y-6">
                <div className="flex gap-2 justify-center" onPaste={handlePaste}>
                  {digits.map((d, i) => (
                    <input key={i} ref={el => { inputRefs.current[i] = el }} type="text" inputMode="numeric" maxLength={1}
                      value={d} onChange={e => handleDigitChange(i, e.target.value)} onKeyDown={e => handleKeyDown(i, e)} disabled={loading}
                      className={`w-12 h-14 text-center text-2xl font-bold rounded-xl border bg-secondary/60 focus:outline-none focus:ring-2 focus:ring-primary/60 transition-all ${d ? 'border-primary/50' : 'border-border/60'} ${loading ? 'opacity-50' : ''}`} />
                  ))}
                </div>
              ...
              <div className="mt-6 text-center">
                ...
                {countdown > 0 ? (
                  <p className="text-xs text-muted-foreground">Resend in <span className="text-primary font-mono">{countdown}s</span></p>
                ) : (
                  <button onClick={handleResend} disabled={resending}
                    className="flex items-center gap-1.5 text-sm text-primary hover:underline mx-auto disabled:opacity-50">
                    {resending ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Sending…</> : <><RefreshCw className="w-3.5 h-3.5" />Resend code</>}
                  </button>
                )}
              </div>
              <p className="text-center text-xs text-muted-foreground mt-4">
                Wrong account? <Link href="/register" className="text-primary hover:underline">Register again</Link>
              </p>
            </>
          )}
        </div>
